...
```{r}
sample_sf <- sample %>%
  left_join(basin_sample, by = c("shale_play" = "Shale_play")) %>%
  st_as_sf() %>%
  # convert to points
  st_centroid() %>%
  # edit lithology typo
...
sample_app <- sample_sf %>%
  ...
  select(shale_play, well_id, n_samples, min_days_since_frack, max_days_since_frack) %>%
...
```{r}
#save just new sample df and new play shapefile (rename)
play_sample <- basin_sample
save(play_sample, file = "data/play_sample.RData")
save(sample_app, file = "data/sample_app.RData")
